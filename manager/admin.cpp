...
void admin::on_btn13_clicked()
{
    this->close();
}

void admin::on_btn12_clicked()
{
...
//    //!-- 得到原文件的后缀名
//    QString suffix_save = QFileInfo(filename).suffix();
//    //!--- 设置过滤匹配文件格式
//    QString setFilter = "image(*.jpg *.bmp);;";
//    //!--- selectFilter为弹出对话框中的文件格式，dirString为文件路径，这里显示文件名
//    QString selectFilter,dirString;

//    if(filename.isEmpty() ) return;
//    else dirString = QFileInfo(filename).fileName();
...
//    if( saveFileName.isEmpty() ) QMessageBox::warning(this,tr("error"),tr("error"));
...
//    //!--- 复制文件
//    bool copy_error =file.copy( filename,saveFileName );
